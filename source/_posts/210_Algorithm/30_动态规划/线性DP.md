...
> 注：“线性”一词修饰的绝不是状态数组的维度 ( 非一定是一维的，也不是复杂度为 $O(n)$ )，理论上它指代的是状态 DAG 图的拓扑结构。

线性状态DP定义状态时经常会考虑 ”某类有序事件中，前若干子事件的答案“（一般来说是时间轴）。
...
- **求解方法**：
    ...
    - **贪心** $O(N \log N)$： 维护一个单调数组，同样长度的上升序列，结尾元素越小，后续接纳潜力越大，利用二分优化。
