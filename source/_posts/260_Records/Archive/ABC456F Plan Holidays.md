...
> 数据规模满足 $N,K\le 2\times 10^5$，需要避免枚举每个长度为 $K$ 的窗口。
...
第三个性质的观察源于第一个样例：当 $K=2$ 且代价为 $3,1,4,1,5$ 时，可以买下位置 $2,4$，再免费得到位置 $3$，直接凑出长度为 $3$ 的区间。再往后推导，任何长度至少为 $K+2$ 的合法区间，都可以从边缘截出一个长度为 $K$ 或 $K+1$ 的方案，代价不会更劣。

### 解法零 DP $O(NK)$ - TLE

由于每个点只有 买 和 不买 两种状态，且 当前状态只与上一个点的状态 有关，性质 2，这显然是个动态规划。

记 $dp[i][0]$ 为当前位置不买，记 $dp[i][1]$ 为当前位置购买。由于不能连续两天不买，所以转移为 $dp[i][0]=dp[i-1][1]$ 与 $dp[i][1]=\min(dp[i-1][0],dp[i-1][1])+a[i]$，两种状态分别对应不买与购买当前位置。
...
然后构建状态矩阵，把状态写成列向量 $v_i=(dp[i][0],dp[i][1])^{\mathsf T}$，并寻找满足 $v_i=M_i\otimes v_{i-1}$ 的转移矩阵。原转移可以改写为 $dp[i][0]=\min(\infty+dp[i-1][0],0+dp[i-1][1])$ 与 $dp[i][1]=\min(a[i]+dp[i-1][0],a[i]+dp[i-1][1])$，所以得到 $M_i=\begin{bmatrix}\infty&0\\a[i]&a[i]\end{bmatrix}$ 这一矩阵。

既然有了状态转移矩阵，要求某一段区间 $[L+1,R-1]$ 的 DP 结果，其实就是求这段区间对应的矩阵连乘积。由于矩阵乘法满足结合律，可以用线段树维护每段区间的复合转移。

这里使用列向量，所以先经过左区间、再经过右区间时，父转移应为“右转移乘左转移”。代码中的 `merge(L,R)` 因此计算 $R\otimes L$，若直接写成 $L\otimes R$，就会把状态执行顺序颠倒。
...
Mat merge( const Mat &L, const Mat &R ) {
    ...
    for( int i = 0; i < 2; ++ i ) {
        for( int j = 0; j < 2; ++ j )
            for( int k = 0; k < 2; ++ k ) {
                if( R.m[i][k] == inf || L.m[k][j] == inf ) continue;
                res.m[i][j] = min( res.m[i][j], R.m[i][k] + L.m[k][j] );
            }
    }
    return res;
}
...
- 归属：[[浅谈线段树信息合并|线段树信息合并]]、DDP、广义矩阵乘法、双栈维护可结合信息。
- 新增：保留从朴素 DP 到 min-plus 矩阵、线段树区间复合，再到双栈滑动窗口的完整算法演化；矩阵相乘顺序必须与状态向量约定一致。
- 分类：长期独立保留。
