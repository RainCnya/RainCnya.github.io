...
- **复杂度分析**：$O(M \log M + N)$。瓶颈在于 Dijkstra。
    
- **碎碎念**：最短路树 (SPT) 是一个最短路的模型。很多“删边保持最短路”或者“限边数求最短路覆盖”的问题，都可以从 SPT 的角度切入。这道题麻烦的地方是 $k$ 的限制，必须通过 BFS 保证保留的边是连通的，否则你保留了叶子的边但没留祖先的边，叶子到根的距离还是会变。
