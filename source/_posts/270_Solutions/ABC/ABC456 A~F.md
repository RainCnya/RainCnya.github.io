...
建模完成后，回归问题本质。题目要求能“永远”放假，在一个只有 $N \times W$ 个有限节点的有向图里走无限步，根据鸽巢原理，意味着路径必定走入了一个**环（Cycle）**！ 所以我们只需要在建好的图上做一遍 DFS 搜环即可。给每个到达的点打上访问标记（正在访问 / 已访问完毕），如果在搜索过程中碰到“正在访问”的节点，就说明成功找到了环。
...
2. **不能有连续的不买**：如果有连续两天都不买（即出现 `0 0` 的状态），它们会互相卡住，死锁导致谁也无法免费变成假期。
3. 只需要考虑区间长度为 $K$ 和 $K + 1$ 即可。

第三个性质的观察源于第一个样例： ```text 5 2 3 1 4 1 5 res = 2 ``` 在这个样例中，可以买下 $[2,4]$ 两天，然后 $[3]$ 就免费了，直接凑出了长度为 3 的区间。再往后推导，考察更长的区间（比如 $K+2$）有意义吗？其实并没有。任何 $\geq K+2$ 的合法区间，我们都可以通过去掉最边缘的元素，无损降级转换成 $K$ 和 $K + 1$ 长度的情况，且代价不会变得更劣。
