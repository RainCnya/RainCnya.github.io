...
- **4.1 Dilworth与降维**
	...
	- [[P1020 导弹拦截]] —— **Dilworth 定理应用**。
    
	- [[ABC439E Kite]] —— **二维偏序转化**。
    
	- [[CF2193F Pizza Delivery]] —— **单向性降维**。
 
- **4.2 状态同步与去重计数**

	- [[P1006 传纸条]] —— **多路径同步**。
    
	- [[P2516 最长公共子序列]] —— **计数类 DP**。
    
- **4.3 转移优化**

	- [[ABC442F Diagonal Separation 2]] —— **后缀和优化**。
    
	- [[TZ9547 Silly Xiao Zhang]] —— **单调栈 + 后缀和**。

- **4.4 复杂约束与离散化**
	
	- [[P1052 过河]] —— **路径压缩**。
    
	- [[ABC443E Climbing Silver]]—— **复杂状态设计**。
        
- **关联笔记**:
    
    - [[树状数组]]: LIS 的 $O(N \log N)$ 实现同样可以通过权值树状数组实现。
        
    - [[区间DP]]: 线性序列向内部折叠、对称性转移的进阶模型。

	- [[单调队列优化]]: 针对 $f[i] = \min\{f[j]\} + cost[i]$ 型转移的通用工具。