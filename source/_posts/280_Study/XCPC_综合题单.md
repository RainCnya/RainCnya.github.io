...
> 本 Part 按“状态模型 → 结构型 DP → 优化型 DP → 高阶与综合”组织。基础题保留但折叠；同一道题允许因知识交叉在其它 Part 再出现。

### Part 4.1 线性 DP / 序列模型｜主线
...
- [ ] [P1775 石子合并（弱化版）](https://www.luogu.com.cn/problem/P1775) `衔接`：作为 P1880 的前置。
- [ ] [P1005 矩阵取数游戏](https://www.luogu.com.cn/problem/P1005) `复盘`：区间取数模型，保留作补充。
...
- [ ] [P1040 加分二叉树](https://www.luogu.com.cn/problem/P1040) `主练`：区间 DP 与方案恢复。
...
- [ ] [CF149D Coloring Brackets](https://www.luogu.com.cn/problem/CF149D) `挑战`：括号结构与区间计数。
...
- [ ] [P5851 Greedy Pie Eaters P](https://www.luogu.com.cn/problem/P5851) `挑战`：区间贡献与最优划分。
...
- [ ] [P2585 三色二叉树](https://www.luogu.com.cn/problem/P2585) `主练`：树形状态分类。
...
- [ ] [P2515 软件安装](https://www.luogu.com.cn/problem/P2515) `挑战`：SCC 缩点后依赖背包。
...
- [ ] [P4316 绿豆蛙的归宿](https://www.luogu.com.cn/problem/P4316) `衔接`：DAG 上期望递推。
- [ ] [P2656 采蘑菇](https://www.luogu.com.cn/problem/P2656) `主练`：SCC 与 DAG 最优路径。
- [ ] [P7077 函数调用](https://www.luogu.com.cn/problem/P7077) `主练`：调用图上的贡献传播。
- [ ] [P3953 逛公园](https://www.luogu.com.cn/problem/P3953) `主练`：最短路结构上的计数 DP。
...
- [ ] [P2622 关灯问题 II](https://www.luogu.com.cn/problem/P2622) `衔接`：状态图最短操作次数。
...
- [ ] [P1725 琪露诺](https://www.luogu.com.cn/problem/P1725) `衔接`：窗口最值优化 DP 的标准入口。
- [ ] [P1714 切蛋糕](https://www.luogu.com.cn/problem/P1714) `衔接`：前缀信息与滑动窗口最值。
- [ ] [P2034 选择数字](https://www.luogu.com.cn/problem/P2034) `主练`：从朴素 DP 过渡到窗口优化。
- [ ] [P2627 Mowing the Lawn G](https://www.luogu.com.cn/problem/P2627) `复盘`：与 P2034 模型接近。
- [ ] [P3572 PTA-Little Bird](https://www.luogu.com.cn/problem/P3572) `主练`：多组窗口范围下的 DP 优化。
- [ ] [P3800 Power 收集](https://www.luogu.com.cn/problem/P3800) `主练`：分层 DP 与滑动窗口最值。
- [ ] [P3957 跳房子](https://www.luogu.com.cn/problem/P3957) `主练`：参数判定、DP 与单调队列。
...
- [ ] [P2569 股票交易](https://www.luogu.com.cn/problem/P2569) `挑战`：多状态与交易限制。
...
- [ ] [P2866 [USACO06NOV] Bad Hair Day S](https://www.luogu.com.cn/problem/P2866) `衔接`：经典单调栈计数。
- [ ] [P1823 Patrik 音乐会的等待](https://www.luogu.com.cn/problem/P1823) `主练`：单调栈处理相等元素与可见关系。
...
- [ ] [P1714 切蛋糕](https://www.luogu.com.cn/problem/P1714) `复盘`：前缀和 + 单调队列；DP 视角已在 Part 4 体现。
- [ ] [P2251 质量检测](https://www.luogu.com.cn/problem/P2251) `复盘`：固定窗口最小值，作为单调队列基本应用。
...
- [ ] [P1816 忠诚](https://www.luogu.com.cn/problem/P1816) `衔接`：静态区间最小值。
...
- [x] [P1908 逆序对](https://www.luogu.com.cn/problem/P1908)
- [ ] [P1966 火柴排队](https://www.luogu.com.cn/problem/P1966) `复盘`：排名映射后的逆序对。
...
- [ ] [P5677 配对统计](https://www.luogu.com.cn/problem/P5677) `挑战`：预处理贡献关系后转离线区间统计。
...
- [ ] [P6492 [COCI 2010/2011 #6] STEP](https://www.luogu.com.cn/problem/P6492) `主练`：维护连续段信息。
...
- [ ] [P3870 [TJOI2009] 开关](https://www.luogu.com.cn/problem/P3870) `主练`：区间翻转与区间计数。
...
- [ ] [P2709 小B的询问](https://www.luogu.com.cn/problem/P2709) `衔接`：普通莫队的频率平方和练习。
...
### Part 7.21 树链剖分 / 树上路径数据结构｜主线
只保留真正以树链剖分、路径数据结构或圆方树 + 数据结构为核心的题；纯 LCA、树上差分、复杂树论题留给 Part 8。
...
### Part 7.22 Small-to-Large / 集合启发式合并｜进阶主线
...
### Part 7.23 树上启发式合并 / DSU on Tree｜进阶主线
...
### Part 7.28 时间线段树 / 可撤销数据结构｜进阶主线
这里只保留数据结构技术本身的代表题；完整的离线动态连通训练链仍放在图论 Part 8。
...
### Part 7.29 动态树 / Link-Cut Tree｜地图保留
...
> **模板与基本性质**
- [ ] [P3371 单源最短路径（弱化版）](https://www.luogu.com.cn/problem/P3371) `衔接`：统一松弛过程与最短路状态。
...
- [ ] [P1119 灾后重建](https://www.luogu.com.cn/problem/P1119) `主练`：理解 Floyd 的阶段含义与动态开放点。
- [ ] [P5905 【模板】Johnson 全源最短路](https://www.luogu.com.cn/problem/P5905) `挑战`：Johnson 全源最短路，处理稀疏图与负权边。

> **建图、状态与分层**
- [ ] [P1346 电车](https://www.luogu.com.cn/problem/P1346) `衔接`：把操作代价准确映射为边权。
...
- [ ] [P1073 [NOIP2009] 最优贸易](https://www.luogu.com.cn/problem/P1073) `主练`：分层状态 / 最短路式 DP。
...
### Part 8.4 路径结构 / 环 / 状态图｜进阶主线
收纳不适合继续塞进普通最短路，但核心仍是“图上路径结构或状态图”的题。
- [ ] [P6175 无向图的最小环问题](https://www.luogu.com.cn/problem/P6175) `主练`：无向图最小环。
...
- [ ] [P2149 Elaxia 的路线](https://www.luogu.com.cn/problem/P2149) `主练`：分析两组最短路结构的公共部分。
- [ ] [P5837 Milk Pumping G](https://www.luogu.com.cn/problem/P5837) `主练`：路径收益与代价共同出现。
- [ ] [P3489 WIE-Hexer](https://www.luogu.com.cn/problem/P3489) `挑战`：最短路状态中携带通行能力。

### Part 8.5 二叉树 / 递归树结构基础｜地图保留
...
- [ ] [P5536 核心城市](https://www.luogu.com.cn/problem/P5536) `挑战`：围绕树的核心部分进行选择。
- [ ] [P2726 树的双中心](https://www.luogu.com.cn/problem/P2726) `挑战`：从单中心扩展到双中心。

### Part 8.7 树的重心 / 贡献统计｜主线
- [ ] [P1395 会议](https://www.luogu.com.cn/problem/P1395) `衔接`：树重心经典题。
- [ ] [P5666 树的重心](https://www.luogu.com.cn/problem/P5666) `主练`：删除点后的重心与连通块大小分析。
...
### Part 8.8 LCA / 树上差分 / 路径关系｜主线

只保留真正围绕祖先关系、路径差分与静态路径统计的题；在线路径数据结构留在 Part 7。
...
- [ ] [P1600 天天爱跑步](https://www.luogu.com.cn/problem/P1600) `挑战`：路径、深度与时间条件共同参与统计。
- [ ] [P3038 [USACO11DEC] Grass Planting G](https://www.luogu.com.cn/problem/P3038) `主练`：树上差分 / 路径增量维护。

### Part 8.9 倍增 / 函数图 / 跳转｜主线
...
### Part 8.11 拓扑排序 / DAG 结构 / 传递闭包｜主线
DAG 上复杂 DP 已由 Part 4 负责；这里保留拓扑序、可达关系与 DAG 结构本身。
...
- [ ] [B3611 【模板】传递闭包](https://www.luogu.com.cn/problem/B3611) `衔接`：传递闭包模板。
- [ ] [P2419 Cow Contest S](https://www.luogu.com.cn/problem/P2419) `主练`：可比较关系与传递闭包统计。
- [ ] [P3573 RAJ-Rally](https://www.luogu.com.cn/problem/P3573) `挑战`：DAG 上删除一个点后的最长路径。

### Part 8.12 差分约束 / 特殊最短路｜主线
- [ ] [P5960 【模板】差分约束算法](https://www.luogu.com.cn/problem/P5960) `衔接`：差分约束标准模板。
...
- [x] [P3199 最小圈](https://www.luogu.com.cn/problem/P3199) `主练`：最小平均环。
- [ ] [P2371 墨墨的等式](https://www.luogu.com.cn/problem/P2371) `主练`：同余类上的最短路。

### Part 8.13 强连通分量 / 缩点 DAG｜主线
...
- [ ] [P2002 消息扩散](https://www.luogu.com.cn/problem/P2002) `主练`：缩点后统计源 SCC。
- [x] [P2746 [USACO5.3] Network of Schools](https://www.luogu.com.cn/problem/P2746)
- [ ] [P2812 校园网络](https://www.luogu.com.cn/problem/P2812) `主练`：缩点 DAG 的入度与出度综合。
...
- [ ] [P2515 软件安装](https://www.luogu.com.cn/problem/P2515) `挑战`：SCC 缩点后接依赖背包，作为图论与 DP 交叉。
...
图论语义以这里为主；Part 7 只保留时间线段树 / rollback 技术本身的代表题。
- [ ] [P5787 线段树分治 / 二分图](https://www.luogu.com.cn/problem/P5787) `衔接`：时间线段树与可撤销并查集入口。
